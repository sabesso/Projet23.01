...
<body style="background-image: url(images/img-glob.jpg); background-repeat:no-repeat; background-attachment: fixed;">
    <div class="container bg-light">
        <div class="mt-5 w-100 h-100 pt-3">
            <a href="http://www.jeuxjeuxjeux.fr/jeu/bike-ride.html" target="_blank"><img class="img-fluid mx-auto d-block w-100" src="images/motobike.jpg" alt="jeuxVoiture"></a>
<!-- _-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_ -->
            <div class="card-img">
                <h1 class="card-title text-center" style="text-shadow: 3px 2px lightgray"><a class="text-primzry text-decoration-none" href="http://www.jeuxjeuxjeux.fr/jeu/bike-ride.html" target="_blank">Cliquez pour jouer</a></h1>
            </div>
        </div>
<!-- _-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_ -->
        <div class="text mt-5" >
                    ...
                    <?php if((!empty($_GET['update'])) && ($data['id'] == $_GET['update'])): ?>
                        <form action="?action=motobike.php&update=<?= $data["id"] ?>" method="POST">
                            <div class="form-group mt-4">
                                ...
                                <a href="?action=motobike.php" class="btn btn-outline-danger btn-sm">Annuler</a>
                            </div>
                        </form>
                        ...
                        <?php if(empty($_GET["update"]) && isset($_SESSION["userInfo"])) : ?>
                            <div class="">
                                ...
                                <?php if($data["likeSelected"] == 1) : ?>
                                    <a href="?action=motobike.php&update=<?= $data["id"] ?>&like=<?= $data["LikeNumber"] ?>&likeSelected=<?= $data["likeSelected"] ?>" class="btn btn-primary btn-sm px-4">
                                        <img src="https://img.icons8.com/ultraviolet/20/000000/good-quality.png"> J'aime
                                    </a>
                                <?php else : ?>
                                    <a href="?action=motobike.php&update=<?= $data["id"] ?>&like=<?= $data["LikeNumber"] ?>&likeSelected=<?= $data["likeSelected"] ?>" class="btn btn-outline-light btn-sm px-4">
                                        <img src="https://img.icons8.com/ultraviolet/20/000000/good-quality.png"> J'aime
                                    </a>
                                <?php endif ?>
                                <!-- Heart icon -->
                                <?php if($data["heartSelected"] == 1) : ?>
                                    <a href="?action=motobike.php&update=<?= $data["id"] ?>&heart=<?= $data["HeartNumber"] ?>&heartSelected=<?= $data["heartSelected"] ?>" class="btn btn-danger btn-sm px-4">
                                        <img src="https://img.icons8.com/flat_round/20/000000/hearts.png"> J'adore
                                    </a>
                                <?php else : ?>
                                    <a href="?action=motobike.php&update=<?= $data["id"] ?>&heart=<?= $data["HeartNumber"] ?>&heartSelected=<?= $data["heartSelected"] ?>" class="btn btn-outline-light btn-sm px-4">
                                        <img src="https://img.icons8.com/flat_round/20/000000/hearts.png"> J'adore
                                    </a>
                                <?php endif ?>
                                <?php if ($_SESSION["userInfo"]["Username"] == $data["Username"]) : ?>
                                    <!-- Modify the comment -->
                                    <a href="?action=motobike.php&update=<?= $data["id"] ?>" class="btn btn-outline-light btn-sm px-4">
                                        <img src="https://img.icons8.com/ios/20/000000/edit-chat-history.png"> Modifier
                                    </a>
                                    <!-- Delete the comment -->
                                    <a href="?action=motobike.php&delete=<?= $data["id"] ?>" class="btn btn-outline-light btn-sm px-4">
...
<!-- Shows the sub comments ------------------------------------------------------- -->
                        ...
                        <?php endforeach ?>
                        <?php if(empty($_GET["update"]) && isset($_SESSION["userInfo"])) : ?>
                            <form class="mb-3 form-inline" action="?action=motobike.php&subcomment=<?= $data["id"] ?>" method="POST">
        ...
        <div class="row mt-3">
            <div class="col-md-6 offset-md-3"> 
                <?php if(!isset($_GET["update"])): ?>
                    ...
                    <form action="?action=motobike.php&add=3" method="POST" enctype="multipart/form-data">
